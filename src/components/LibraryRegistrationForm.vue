 ...
 */
 const validateConfirmPassword = (blur) => {
  if (formData.value.password !== formData.value.confirmPassword) {
    if (blur || formData.value.confirmPassword !== '') {
...
const validateReasonWithWord = (blur) => {
  if (formData.value.reason.includes('friend')) {
    if (blur || formData.value.reason !== '') {
      errors.value.reason = 'Grate to have a friend.';
    } 
  } else {
    errors.value.reason = null
  }
        ...
        <form @submit.prevent="submitForm">
          ...
          <div class="mb-3">
            ...
            <textarea
              ...
              @input="() => {validateReason(false); validateReasonWithWord(true)}"
            ></textarea>
